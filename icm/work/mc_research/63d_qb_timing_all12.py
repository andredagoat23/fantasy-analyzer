...
*** RETRACTED — 12-slot version of the retracted 63_ design. Same bimodal-median error. See L57 and 64_.
*** It also WROTE data/qb_timing.csv; that file has been deleted. Do NOT regenerate it from this script.
...
OUT = os.path.join(HERE, "results_63d_qb_timing_all12.txt")
...
if __name__ == "__main__":
    ...
    # emit the committed lookup the app would read
    import csv
    tbl = os.path.join(os.path.dirname(HERE), "..", "..", "data", "qb_timing.csv")
    tbl = os.path.abspath(tbl)
    with open(tbl, "w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["slot", "teams", "natural_round", "best_round", "gain_vs_natural", "ci95",
                    "early_cost_R4", "late_cost", "n_seeds", "verdict"])
        for sl in SLOTS:
            ds = d[d.slot == sl]
            base = ds[ds.arm == "natural"].set_index("seed")
            best, bv, bse = None, -1e9, 0
            ...
            for k, v in ARMS:
                ...
                if diff.mean() > bv:
                    best, bv, bse = v, diff.mean(), diff.std()/np.sqrt(len(diff))
            clears = bv - 1.96*bse > 0
            w.writerow([sl, TEAMS, int(base.qb_round.median()), best, round(bv,1),
                        round(1.96*bse,1), round(r4,1) if r4 is not None else "",
                        "", N, "USE" if clears else "NOISE"])
    say(f"wrote lookup table -> {tbl}")
